...
static string OUT_FILE_COL_HEADERS =
    "Frame,Timestamp (ms),x,y,Active points,"\
...
int main(int argc, char **argv)
{
    ...
    if (output_flag)
    {
        ...
        float max = 0.0;
        int argmax = 0;
        for (int i=0; (unsigned)i<cmt.points_active.size(); i++)
            if (cmt.points_active[i].y > max){
                max = cmt.points_active[i].y;
                argmax = i;}

        output_file << cmt.points_active[argmax].x << ",";
        output_file << cmt.points_active[argmax].y << ",";
